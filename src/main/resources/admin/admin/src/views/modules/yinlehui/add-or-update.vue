<template>
	<div class="addEdit-block" :style='{"padding":"30px 0"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="音乐会名称" prop="yinlehuimingcheng">
					<el-input v-model="ruleForm.yinlehuimingcheng" placeholder="音乐会名称" clearable  :readonly="ro.yinlehuimingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="音乐会名称" prop="yinlehuimingcheng">
					<el-input v-model="ruleForm.yinlehuimingcheng" placeholder="音乐会名称" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="演出时间" prop="yanchushijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.yanchushijian" 
						type="datetime"
						:readonly="ro.yanchushijian"
						placeholder="演出时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.yanchushijian" label="演出时间" prop="yanchushijian">
					<el-input v-model="ruleForm.yanchushijian" placeholder="演出时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="结束时间" prop="jieshushijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.jieshushijian" label="结束时间" prop="jieshushijian">
					<el-input v-model="ruleForm.jieshushijian" placeholder="结束时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="音乐厅" prop="yinleting">
					<el-select :disabled="ro.yinleting" v-model="ruleForm.yinleting" placeholder="请选择音乐厅" >
						<el-option
							v-for="(item,index) in yinletingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="音乐厅" prop="yinleting">
					<el-input v-model="ruleForm.yinleting"
						placeholder="音乐厅" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.selected" label="已选座位[用,号隔开]" prop="selected">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#797979","fontWeight":"500","display":"inline-block"}'>{{ruleForm.selected}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-if="type!='info'"  label="演出信息" prop="yanchuxinxi">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.yanchuxinxi" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.yanchuxinxi" label="演出信息" prop="yanchuxinxi">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#797979","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.yanchuxinxi"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				yinleting: [
					{ required: true, message: '音乐厅不能为空', trigger: 'blur' },
				],
				yanchuxinxi: [
				],
				thumbsupnum: [
	...
	created() {
		this.ruleForm.yanchushijian = this.getCurDateTime()
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='yinleting'){
							this.ruleForm.yinleting = obj[o];
							this.ro.yinleting = true;
							continue;
						}
						if(o=='yanchuxinxi'){
							this.ruleForm.yanchuxinxi = obj[o];
							this.ro.yanchuxinxi = true;
							continue;
						}
						if(o=='thumbsupnum'){
            ...
            this.$http({
				url: `option/yinleting/yinletingmingcheng`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.yinletingOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.yanchuxinxi = this.ruleForm.yanchuxinxi.replace(reg,'../../../springboot9z622/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yinlehui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yinlehuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
